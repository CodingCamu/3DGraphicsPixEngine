...
X::Color DirectionalLight::ComputeLightColor(const Vector3& position, const Vector3& normal)
{
	...
	Vector3 dirToLight = (mDirection) * -1;
...
X::Color PointLight::ComputeLightColor(const Vector3& position, const Vector3& normal)
{
	...
	float atten = 1.0f / (mAttenConstant + (mAttenLinear + distToLight) + (mAttenQuadratic * distToLight * distToLight));
...
X::Color SpotLight::ComputeLightColor(const Vector3& position, const Vector3& normal)
{
	...
	Vector3 lightDirection =(dirToLight)* -1;
	...
	float atten = spot / (mAttenConstant + (mAttenLinear + distToLight) + (mAttenQuadratic * distToLight * distToLight));
	float iL = X::Math::Clamp(atten, 0.0f, 1.0f);
